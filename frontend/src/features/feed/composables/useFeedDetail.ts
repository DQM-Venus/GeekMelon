...
import type { ApiResponse, FeedDetailRecord } from '@/features/feed/types'

const API_BASE_URL = import.meta.env.VITE_API_BASE_URL ?? '/api'

export function useFeedDetail() {
  const selectedId = shallowRef<number | null>(null)
  const detail = shallowRef<FeedDetailRecord | null>(null)
  ...
  async function openDetail(id: number) {
    ...
    try {
      ...
      const payload = (await response.json()) as ApiResponse<FeedDetailRecord>

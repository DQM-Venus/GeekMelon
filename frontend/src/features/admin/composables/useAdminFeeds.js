...
export function useAdminFeeds() {
    ...
    async function fetchFeeds() {
        ...
        try {
            const result = await adminFetch(`/admin/feeds?${params.toString()}`);
    ...
    async function openDetail(id) {
        ...
        try {
            detail.value = await adminFetch(`/admin/feeds/${id}`);
        }
        catch (error) {
            detail.value = null;
    ...
    async function hideFeed(id) {
        saving.value = true;
        try {
            detail.value = await adminFetch(`/admin/feeds/${id}/hide`, {
                method: 'POST',
            });
            await fetchFeeds();
        }
    ...
    async function restoreFeed(id) {
        saving.value = true;
        try {
            detail.value = await adminFetch(`/admin/feeds/${id}/restore`, {
                method: 'POST',
            });
            await fetchFeeds();
        }
